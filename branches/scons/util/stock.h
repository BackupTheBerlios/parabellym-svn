...
#ifndef __faeutil_stock_h
#define __faeutil_stock_h
...
extern "C" unsigned int faeutil_random(unsigned int);
...
stock<T>::stock(unsigned int pgsz)
{
	...
	cnt_data = 0;
	cnt_free = 0;
#if 1 || defined(NO_RANDOM_ID)
	rnd_seed = 0;
#else
	rnd_seed = faeutil_random(~0) & 0x7fffffff;
#endif
	pagesize = pgsz;
}
